# Vault of Whisper Campaign — Wiki Index

**Last Updated:** 2026-08-01  
**Total Pages:** 8  
**Ingest Source:** Eberron TiddleWiki (`sources/eberron/eberron_tw/`)

---

## Page Counts by Category

| Category | Count |
|----------|-------|
| Entities (NPCs) | 3 |
| Locations | 3 |
| Mechanics | 2 |
| **Total** | **8** |

---

## Recent Changes

### 2026-08-01 — Weekly Check (0 new pages)

No new or updated tiddlers found in `sources/eberron/eberron_tw/`. All 8 source tiddlers carry `modified:` timestamps predating the 2026-05-23 ingest. Wiki is current.

### 2026-07-25 — Weekly Check (0 new pages)

No new or updated tiddlers found in `sources/eberron/eberron_tw/`. All 8 source tiddlers carry `modified:` timestamps predating the 2026-05-23 ingest. Wiki is current.

### 2026-07-18 — Weekly Check (0 new pages)

No new or updated tiddlers found in `sources/eberron/eberron_tw/`. All 8 source tiddlers carry `modified:` timestamps predating the 2026-05-23 ingest. Wiki is current.

### 2026-07-11 — Weekly Check (0 new pages)

No new or updated tiddlers found in `sources/eberron/eberron_tw/`. All 8 source tiddlers carry `modified:` timestamps predating the 2026-05-23 ingest. Wiki is current.

### 2026-07-04 — Weekly Check (0 new pages)

No new or updated tiddlers found in `sources/eberron/eberron_tw/`. All 8 source tiddlers carry `modified:` timestamps predating the 2026-05-23 ingest. Wiki is current.

### 2026-06-27 — Weekly Check (0 new pages)

No new or updated tiddlers found in `sources/eberron/eberron_tw/`. All 8 source tiddlers carry `modified:` timestamps predating the 2026-05-23 ingest. Wiki is current.

### 2026-06-20 — Weekly Check (0 new pages)

No new or updated tiddlers found in `sources/eberron/eberron_tw/`. All 8 source tiddlers carry `modified:` timestamps predating the 2026-05-23 ingest. Wiki is current.

### 2026-06-13 — Weekly Check (0 new pages)

No new or updated tiddlers found in `sources/eberron/eberron_tw/`. All 8 source tiddlers carry `modified:` timestamps predating the 2026-05-23 ingest. Wiki is current.

### 2026-06-06 — Weekly Check (0 new pages)

No new or updated tiddlers found in `sources/eberron/eberron_tw/`. All 8 source tiddlers carry `modified:` timestamps predating the 2026-05-23 ingest. Wiki is current.

### 2026-05-23 — Initial Ingest (8 pages)

**Entities:**
- [Mordain the Fleshweaver](entities/mordain-the-fleshweaver.md) — excoriate villain, Droaam
- [Lady Elaydren d'Cannith](entities/lady-elaydren-dcannith.md) — House Cannith patron
- [The Whisper](entities/the-whisper.md) — mystery info broker, campaign-central NPC

**Locations:**
- [Sharn](locations/sharn.md) — City of Towers, campaign base
- [The Mournland](locations/the-mournland.md) — destroyed Cyre, hazardous region
- [Wroat](locations/wroat.md) — Brelish capital, political hub

**Mechanics:**
- [Dragonmarks](mechanics/dragonmarks.md) — hereditary magic and house system
- [Manifest Zones](mechanics/manifest-zones.md) — planar geography mechanics

---

## All Pages (A–Z)

- [Dragonmarks](mechanics/dragonmarks.md)
- [Lady Elaydren d'Cannith](entities/lady-elaydren-dcannith.md)
- [Manifest Zones](mechanics/manifest-zones.md)
- [Mordain the Fleshweaver](entities/mordain-the-fleshweaver.md)
- [Sharn](locations/sharn.md)
- [The Mournland](locations/the-mournland.md)
- [The Whisper](entities/the-whisper.md)
- [Wroat](locations/wroat.md)

---

## Campaign Notes

The Vault of Whisper campaign is set in the Eberron setting, shortly after the end of the Last War (996 YK). The party is based in [[Sharn]] and is investigating the [[Vault of Whispers]] — a pre-Galifaran repository of bound extraplanar entities rumored to be located in the [[Mournland]].

Key threads:
- **The Whisper** — who are they, and why do they share the Vault's name?
- **Lady Elaydren's agenda** — what does House Cannith want from the Vault?
- **The Mournland expedition** — can the party reach the Vault and survive it?
